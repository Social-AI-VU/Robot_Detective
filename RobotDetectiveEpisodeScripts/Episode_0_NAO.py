r"""
Robot Detective — Episode 0 (NAO)
=================================
NAO-specific variant of Episode_0.py. This version uses:
...
Then start the episode with:
    ...
    python RobotDetectiveEpisodeScripts/Episode_0_NAO.py
...
INDEX_NAME = "episode_0_robin_docs"
INGEST_DOCS = False
...
def populate_missing_character_definitions(dialogs) -> None:
    ...
    for dialog in dialogs:
        ...
        if missing_in_dialog:
            unresolved.append(f"{dialog.get('id', '<unknown>')}: {', '.join(sorted(missing_in_dialog))}")

    if unresolved:
        raise ValueError("Dialogs reference characters without definitions: " + "; ".join(unresolved))
...
def print_startup_checks() -> None:
    ...
    try:
        ...
        rag_blocks = [d for d in all_dialogs if d.get("type") == "llm_based" and d.get("rag_enabled")]
        ...
        for block in rag_blocks:
            print(f"[CHECK] {block.get('id')} -> index={block.get('index_name', DEFAULT_RAG_INDEX_NAME)}")
...
if __name__ == "__main__":
    ...
    if MANUAL_MODE:
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", suffix="_episode0_nao_manual_dialogs.json", delete=False, encoding="utf-8"
        ) as temp_file:
